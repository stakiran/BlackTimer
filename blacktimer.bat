...
rem BlackTimer v0.1.0
rem ================

set waitmin=%1
set msg=%*

set selfdir_with_delim=%~dp0
set datafoldername=.blacktimer
set datafolderfull=%selfdir_with_delim%%datafoldername%
set dataext=.txt
...
if "%waitmin%"=="" (
	set /p waitmin="[Step 1/2] 何分後にリマインドしますか？ >>> "
)

if "%msg%"=="" (
	set /p msg="[Step 2/2] リマインド時のメッセージは？(空欄可) >>> "
)

rem Re-check for 'set /p'
if "%waitmin%"=="" (
	set waitmin=0
)
if "%msg%"=="" (
